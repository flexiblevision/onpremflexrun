...
def load_valid_commands():
    """TCP presets, indexed by the command string a neighbour sends."""
    return {preset['ioVal']: preset
            for preset in io_ref.find({'ioType': 'TCP'})}


def load_tcp_config():
    return config_ref.find_one({'type': 'tcp_config'})


def help_payload(valid_commands):
    return {
        "commands": {
...
def handle_connection(connections, client_address, valid_commands=None, config=None):
    """Serve one accepted connection until the neighbour disconnects."""
    ...
    while True:
        try:
            data = connections.recv(RECV_BYTES)
            print('received: ', data)
            if data:
                try:
                    ...
                    if incoming_command == "help":
                        b_help_string = json.dumps(help_payload(valid_commands)).encode('utf-8')
...
def create_server(host=SERVER_HOST, port=SERVER_PORT):
...
def serve_forever(sock):
    while True:
        ...
        try:
            handle_connection(connections, client_address)
        finally:
            connections.close()


def main():
    serve_forever(create_server())
